...
# マニピュレータの手先位置の取得
def get_manipulator_pose(L1, L2, theta1, theta2):
    X1 = L1*np.cos(theta1)
    Y1 = L1*np.sin(theta1)
...
def draw_manipulator_operability(L1, L2, theta1, theta2):
    ...
    x1, y1, x2, y2 = get_manipulator_pose(L1, L2, theta1, theta2)
    long_side, short_side, rad = calculate_Ellipsoid_from_Jacobian(J)

    # 楕円の描画
    ellips = pat.Ellipse(xy = (x2, y2), width=long_side*0.5, height=short_side*0.5, angle=np.rad2deg(rad), alpha=0.6, color='red')

    # マニピュレータの描画
    # ax.plot([0, x1], [0, y1], 'k', linewidth=10.0, zorder=1)
    # ax.plot([x1, x2], [y1, y2], 'k', linewidth=10.0, zorder=1)
    link0 = pat.Rectangle(xy=(0, 0), angle=np.rad2deg(np.arctan2(y1, x1)-np.pi/2), width=0.05, height=L1, ec='b')
    link1 = pat.Rectangle(xy=(x1, y1), angle=np.rad2deg(np.arctan2(y2-y1, x2-x1)-np.pi/2), width=0.05, height=L2, ec='b')
    c0 = pat.Circle(xy=(0, 0), radius=0.1, ec='w', fill=True, zorder=2)
    c1 = pat.Circle(xy=(x1, y1), radius=0.1, ec='w', fill=True, zorder=2)
    ax.add_patch(c0)
    ax.add_patch(c1)
    ax.add_patch(link0)
    ax.add_patch(link1)
    ...
    def update(val):
        ...
        ellips.set_center((x2, y2))

        # c1.set_xy((x1, y1))
        c1.set_center((x1, y1))

        link0.set_angle(np.rad2deg(np.arctan2(y1, x1)-np.pi/2))

        link1.set_xy((x1, y1))
        link1.set_angle(np.rad2deg(np.arctan2(y2-y1, x2-x1)-np.pi/2))
